function [ice_thickness] = Gantayat14_whole_inversion(drainage_stats,velocity,slope,inversion_parameters)
...
individual_glaciers_labelled = bwlabel(~isnan(drainage_stats.slope));

%Loop through all basins
for loop = 1:nanmax(individual_glaciers_labelled,[],'all')

    if length(drainage_stats.dem(individual_glaciers_labelled==loop))>1


        elevation_band = drainage_stats.dem(individual_glaciers_labelled==loop);
        ...
        for loop_ELA = 1:length(elevation_brackets)
            ...
            slope_band = nanmean(slope_sampled(...
                (double(individual_glaciers_labelled==loop) +double(drainage_stats.dem>ELA)+double(drainage_stats.dem<=ELA+100))==3 ...
                ),'all');
            ...
            slope_sampled_bands((double(individual_glaciers_labelled==loop)+double(drainage_stats.dem>ELA)+double(drainage_stats.dem<=ELA+100))==3)=slope_band;

            velocity_sampled_bands((double(individual_glaciers_labelled==loop)+double(drainage_stats.dem>ELA)+double(drainage_stats.dem<=ELA+100))==3)=velocity_band;
