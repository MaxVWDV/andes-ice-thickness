...
for loop = 1:nanmax(individual_glaciers_labelled,[],'all')
    ...
    if length(drainage_stats.dem(individual_glaciers_labelled==loop))>1
        ...
        elevation_difference = (max(drainage_stats.dem(individual_glaciers_labelled==loop),[],'all')-min(drainage_stats.dem(individual_glaciers_labelled==loop),[],'all'))/1000;

        if elevation_difference<1.6
            basal_shear_stress(individual_glaciers_labelled==loop) = (0.5+159.8*elevation_difference-43.5*elevation_difference.^2)*1000;
        else
            basal_shear_stress(individual_glaciers_labelled==loop) =150000;
        end
